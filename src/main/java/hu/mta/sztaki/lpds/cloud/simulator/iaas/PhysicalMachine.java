...
public class PhysicalMachine extends MaxMinProvider implements VMManager<PhysicalMachine, ResourceConstraints> {
    ...
    public interface StateChangeListener {
         ...
         *                 delivered
         * @param newState the new state the PM will be in after the event is
         *                 complete
         */
    ...
    public class ResourceAllocation extends DeferredEvent implements VirtualMachine.StateChange {
         ...
         * machine only!
         *
         * @param realAlloc defines the actually reserved resources with the
         *                  allocation
         * @param alloc     defines the promised resources for the user
         * @param until     defines the duration in ticks for which the allocation
         ...
         *
         * @param vm the virtual machine that will use the allocated resources
         *           in the future
         * @throws VMManagementException if the allocation passed to the VM is already expired or
     ...
     * @param perCorePocessing defines the processing capabilities of a single CPU core in
     *                         this machine (in instructions/tick)
     * @param memory           defines the total physical memory this machine has under
     *                         control (in bytes)
     * @param disk             defines the local physical disk & networking this machine has
     *                         under control
     * @param onD              defines the time delay between the machine's switch on and the
     *                         first time it can serve VM requests
     * @param offD             defines the time delay the machine needs to shut down all of
     *                         its operations while it does not serve any more VMs
     * @param powerTransitions determines the applied power state transitions while the
     ...
     * @param disk                defines the local physical disk & networking this machine has
     *                            under control
     * @param turnonOperations    defines the tasks to execute before the PM can be turned on -
     *                            this can be considered as the simulation of the boot process.
     *                            for the complete definition of this array have a look at the
     *                            powerstatedelayer class.
     * @param switchoffOperations defines the tasks to execute before the PM can be switched off
     ...
     *
     * @param migrateHere the physical machine where the currently hosted VMs of this VM
     ...
     * </ul>
     * @throws NetworkException                if the migration would need to use network connections that
     *                                         are not set up correctly.
     * @throws VMManager.VMManagementException if the migration fails because the VM is not in correct state
     ...
     * @param strict                   if the PM should not return an allocation if it cannot
     *                                 completely meet the request.
     * @param allocationValidityLength for how long the PM should keep the allocation in place.
     ...
     * @throws VMManagementException        if the VA is not present on the source, or if the allocation
     *                                      specified is already expired/used, or if the VM cannot be
     *                                      switched on for some reason
     * @throws NetworkNode.NetworkException if the VA is not transferrable from the vaSource to its
     *                                      destination
     */
     ...
     * @param killTasks if the VM must be switched off without caring about its tasks
     ...
     * there is a VM that terminates on the PM)
     *
     * @param e the listener object that no longer expects free capacity
     *          events
     */
